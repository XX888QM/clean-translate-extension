...
- 🎯 **Per-Site Preferences / 网站偏好**
  默认手动翻译；支持「自动翻译全部 / 仅白名单 / 全部手动」三种模式，单个域名可单独设为「自动」或「从不」。
...
| `<all_urls>` content script | 翻译脚本注入任意网页；默认手动，未触发且未开启自动翻译时不发送页面文本 |
...
- 隐私：手动翻译不再暗中把当前网站改成自动翻译
- 隐私政策：补全 Google 划词对比、Google 回退及 MyMemory 回退的真实接收方
- 测试 85 → 92，新增安全边界回归用例

### v1.5.3
- 划词翻译：弹窗新增开关（关闭立即生效）；敏感站（邮箱/网银/内网等）选中文本不再自动外发翻译接口，右键菜单显式翻译不受限
- 敏感站透明化：弹窗"当前网站"显示"敏感站点 · 不自动翻译"并禁用「自动」按钮，明示保护而非故障
- 白名单模式救活：选项改名"仅翻译标「自动」的网站"并附操作提示（原"白名单"无任何添加入口，选了等于全部不翻）
- 目标语言 5 种 → 15 种（法/德/西/俄/葡/巴葡/意/阿/泰/越），与后端白名单一致性由测试锁定
- 修复报错提示打绿勾：error toast 图标改红色，熔断提示改 error 类型
- 快捷键提示新增"修改"入口，一键跳转 Chrome 快捷键设置页
- 页内浮层（悬停原文 / 划词气泡）支持深色模式
- 细节：双语开关提示"重新翻译后生效"；开关/下拉/术语删除按钮补 aria-label；"翻译统计"更名"本页统计"
- 测试 83 → 85（新增 popup 与 background 语言/引擎选项一致性锁定）
